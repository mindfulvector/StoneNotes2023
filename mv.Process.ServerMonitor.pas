    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            mv.Logger.Log('Process has crashed. Restarting...');
      ...
      begin
        mv.Logger.Log('Process handle could not be retrieved. Restarting...');
        StartServer(FProcessInfo.Cmd);
        Continue;       // So we don't restart twice if files also changed
      end;
      ...
      begin
        ...
        begin
          mv.Logger.Log('Change detected. Restarting...');
...
begin
  ...
  begin
    ...
    mv.Logger.Log('check ' + Path);
    if FileExists(Path) then
      Exit(Path);
//    else
//      mv.Logger.Log('error! ' + IntToStr(GetLastError));
...
begin
  mv.Logger.Log(Format('$$$ server_monitor compiled on %s at %s', [DateToStr(Now), TimeToStr(Now)]));

  DenoPath := FindUtilityFile('Support', 'stonenotes_deno.exe');

  if DenoPath = '' then
  begin
    mv.Logger.Log('stonenotes_deno.exe not found');
    Exit;
  end;
  mv.Logger.Log('stonenotes_deno.exe found at ' + DenoPath);

  ServerPath := FindUtilityFile('Services', 'server.ts');
  if ServerPath = '' then
  begin
    mv.Logger.Log('server.ts not found');
    Exit;
  end;
  mv.Logger.Log('server.ts found at ' + ServerPath);
  ...
  Cmd := Format('%s run '    +
    ...
    '--unstable '      +
    '%s '              +
    '"%s" "%s"', [DenoPath, ServerPath, ExtractFileDir(ServerPath), ExtractFileDir(ExtractFileDir(ServerPath)) + '/plugins']);

  mv.Logger.Log('Final command: ' + Cmd);

  mv.Logger.Log('Starting server...');
  StartServer(Cmd);

  mv.Logger.Log('Monitoring server...');
  MonitorServer;
end;
