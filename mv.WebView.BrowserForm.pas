...
uses
  ...
  uWVTypes, System.Win.ComObj, System.Contnrs,
...
//    FServices: TDictionary<String, OleVariant>;
    procedure LoadURL;
...
uses
  FMX.Platform, FMX.Platform.Win, mv.Logger;
...
//procedure TBrowserForm.AddService(AServiceName: string; AService: OleVariant);
//begin
//  if not Assigned(FServices) then
//  begin
//    FServices := TDictionary<String, OleVariant>.Create;
//  end;
//  mv.Logger.Log('Add service `' + AServiceName + '` with ID #' + IntToStr(NativeInt(@AService)) + ' to browser ID #' + Self.Handle.ToString);
...
begin
  try
    ...
    on e : exception do
      mv.Logger.Log('TBrowserForm.CustomWndProc error : ' + e.message);
  end;
end;
...
var
  ...
  Service: OleVariant;
begin
  mv.Logger.Log('WVFMXBrowser1AfterCreated');
  FMXWindowParent.UpdateSize;
  Caption            := 'StoneNotes_PluginHost';
  AddressLay.Enabled := True;
//  if Assigned(FServices) then
//    for ServiceName in FServices.Keys do
//    begin
//      WVFMXBrowser1.AddHostObjectToScript(ServiceName, FServices[ServiceName]);
//    end;
end;
